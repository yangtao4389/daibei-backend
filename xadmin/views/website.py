...
from .base import BaseAdminView, filter_hook, CommAdminView
...
class LogoutView(BaseAdminView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        context = self.get_context()
        xadminExitLocalUrl = request.session.get("xadminExitLocalUrl")
        if xadminExitLocalUrl:
            context['xadminExitLocalUrl'] =xadminExitLocalUrl
            self.request.session.flush()
            return HttpResponseRedirect(xadminExitLocalUrl)  # 可以直接跳出，并且清除session
        defaults = {
            # "xadminExitLocalUrl": xadminExitLocalUrl,
            'extra_context': context,
            # 'current_app': self.admin_site.name,
